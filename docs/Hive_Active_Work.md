...
The supplied production HDataBox/TableInfo source now establishes the host-side root/child relationship, parent-key propagation, required/unique validation, CheckBeforeSave veto, SaveRecord boundary, PerformAfterSave(ID) callback, New/Edit lifecycle, and authoritative reload behavior. These are no longer open discovery questions. Phase 1.14 next-pass implementation must still inspect the exact HControl/IHyperControl and HDataGridView metadata/identity/editing contracts, lookup behavior, host concurrency/version behavior, and the concrete HActionBar contract only where needed before freezing the neutral adapter types. ByControls means same-form controls around the DataGridView; ByForm means an input dialog/form.
