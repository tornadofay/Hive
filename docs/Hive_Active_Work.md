...
The supplied production HDataBox/TableInfo/HDataGridView/AddGrid source now establishes the host-side root/child relationship, parent-key propagation, current single-integer TableInfo key, required/unique validation, CheckBeforeSave veto, SaveRecord boundary, PerformAfterSave(ID) callback, New/Edit lifecycle, authoritative reload behavior, DataTable-backed grid editing, concrete ByForm dialog editing, grid add/edit/delete veto and validation hooks, and child-row in-memory mutation before parent save. These are no longer open discovery questions. Phase 1.14 next-pass implementation must still inspect the exact HControl/IHyperControl metadata/value contract, HDataGridView column metadata and runtime row-to-key mapping, generated/computed-column behavior, complete existing-child edit serialization, lookup behavior, and host concurrency/version behavior. HActionBar is unfinished and should not be used as an authoritative contract. ByControls means same-form controls around the DataGridView; ByForm means an input dialog/form.
