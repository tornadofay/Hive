...
- Example UI is separately centralized for the developer-facing test harness: HiveExampleTestSurface provides the repeated input + copyable C# snippet + Run/status/cancellation/error-handling pattern, while HiveExampleOutputView provides one persistent global output surface shared by examples through IServiceProvider. This is optional composition; specialized examples such as CRUD and dialog demonstrations are free to keep their own UI.
