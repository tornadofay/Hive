...
For the planned Phase 1.14 host-integration example, the Example Host should demonstrate the public neutral host contract rather than a private host-library API. The scenario should use a deterministic fixture adapter and may include native WinForms/custom controls plus an adapter-shaped parent/child data surface with a hidden identity field. It should demonstrate semantic field metadata, operation-scoped stable row identity, generated/computed fields, bound child-data editing, the distinction between direct grid editing, same-form supporting controls, and dedicated editor forms/dialogs, bounded lookup behavior, host validation/business-operation boundaries, UI capability versus Hive authorization, and API/UI capability composition without exposing raw control handles, SQL, or private host implementation details.
