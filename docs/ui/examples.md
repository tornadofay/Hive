...
For Phase 1.14, the Example Host scenario demonstrates the public Hive-owned host contracts and the preferred low-code WinForms integration model. The deterministic fixture uses Hive-owned base WinForms types where appropriate so common control/form integration is automatic, while explicit semantic overrides are used where application meaning cannot be safely inferred. The scenario also retains coverage of the adapter/semantic-provider compatibility path for ordinary/native/custom controls. It uses parent/child data surfaces with a hidden primary-key ID and demonstrates semantic field metadata, stable row identity, generated/computed fields, bounded child-row interaction, direct editor/grid patterns, dependent lookup behavior, host validation/business-operation boundaries, UI capability versus Hive authorization, and API/UI capability composition without exposing raw control handles, SQL, private host classes, or private library APIs.
