...
For the planned Phase 1.14 host-integration example, the Example Host should demonstrate the public Hive-owned host contracts and the contract-first integration model. The deterministic fixture host should implement the contracts itself, with common WinForms behavior handled through the reusable Hive integration infrastructure wherever possible. The scenario may include native/custom controls plus parent/child data surfaces with a hidden primary-key ID. It should demonstrate semantic field metadata, stable row identity, generated/computed fields, bound child-data editing and combined parent/child operations, direct grid and editor-surface patterns, bounded dependent lookup behavior, host validation/business-operation boundaries, UI capability versus Hive authorization, and API/UI capability composition without exposing raw control handles, SQL, private host classes, or private library APIs.
