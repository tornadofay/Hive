...
Questions have `Waiting`, `Answered`, `TimedOut`, and `Cancelled` states. Only the owning RuntimeInstance may wait for or cancel its Question. A responder identity is recorded on the Answer, but Phase 1.6 requires the responder to be the owning RuntimeInstance. Cross-runtime collaboration uses the explicit Delegation channel.
