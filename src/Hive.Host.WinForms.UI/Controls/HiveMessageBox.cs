...
public static class HiveMessageBox
{
    ...
    private static IHiveThemeManager? ResolveThemeManager(IWin32Window? owner)
    {
        if (owner is HiveForm hiveForm)
            return hiveForm.ThemeManager;

        if (owner is Control control && control.FindForm() is HiveForm ownerForm)
            return ownerForm.ThemeManager;
    ...
    private sealed class HiveMessageDialog : Form
    {
        ...
        private int MeasureButtonWidth(string text)
        {
            ...
            return Math.Max(
                112,
                Math.Min(190, measured.Width + 40));
        }
