...
Revision inherits the immediately preceding task, re-audits it for concrete findings, and does not require findings to have been identified in advance. When no Active Work is open, Revision may establish the temporary bounded corrective slice described above before implementation, except for an Architecture task, which remains analysis-only unless implementation is explicitly authorized. `Continue`, `Again`, and `Polish` require an identifiable preceding/current task or active slice and never create new work when none exists. If the preceding context is unavailable or ambiguous, do not guess.
