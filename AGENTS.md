...
- Continue, Revision, Again, and Polish inherit the current task/scope and its permissions. Maintenance does the same when Active Work is open; with no active slice, an explicit bounded corrective Maintenance request may create a temporary slice under the rule below. **Workflow Review** is read-only, may inspect the whole repository beyond the current Active Work boundary, and does not modify repository state or create authorization; it is not the V1/future business Review lifecycle or capability. A Revision of a workflow Review remains read-only unless the user separately authorizes implementation. None advances the roadmap.
