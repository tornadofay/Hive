...
123. Examples must use the shared Hive WinForms UI foundation and must not reference ReaLTaiizor directly. Ordinary operation/test examples should reuse `HiveExampleTestSurface` and the shared `HiveExampleOutputView` through the typed `IHiveExampleOutput` service; specialized UI examples such as CRUD, dialogs, and theme demonstrations may keep their own presentation while reusing applicable Hive controls, layouts, theme contracts, and message surfaces.
