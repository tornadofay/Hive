...
- **Continuation commands do not create work:** `Continue`, `Again`, and `Polish` require an identifiable preceding/current task or active slice and stop when none exists. They do not create a temporary slice or advance the roadmap. `Revision` is the corrective exception described above, but a read-only Workflow Review is not sufficient context to auto-open an implementation slice.
